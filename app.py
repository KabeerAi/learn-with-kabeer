...
# Check if running on Render (PostgreSQL), otherwise fall back to local SQLite
database_url = os.environ.get("DATABASE_URL")

if database_url:
    # Render provides 'postgres://', but Python drivers prefer 'postgresql://'
    if database_url.startswith("postgres://"):
        database_url = database_url.replace("postgres://", "postgresql://", 1)
    app.config["DATABASE"] = database_url
else:
    # Local fallback so your app still runs on your machine
